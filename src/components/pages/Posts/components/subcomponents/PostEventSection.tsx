	...
	if (post.event)
		return (
			...
			>
				...
				{new Date(post.event.startDate).getTime() - new Date().getTime() <
					0 && (
					<Grid item xs={6} lg={2}>
				...
				<Grid item xs={6} lg={2}>
					<Typography style={{ color: 'white' }} align="center">
						{post.event.participants.length}/{post.event.limitParticipants}{' '}
						{t('post.joined')}.
					</Typography>
				</Grid>
				<Grid item xs={4} lg={3}>
															...
															{!eventLoading ? (
																<>
																	<Button
																		size="small"
																		color="primary"
																		onClick={() => handleAdmitToEvent(item._id)}
																	>
																		{t('post.admit')}
																	</Button>
																	<Button
